...
// expects: (dakota{Dakota}, name{String}, schema{Schema}, validations{Validations}, options{Object}?) or (dakota{Dakota}, name{String}, schema{Schema}, null, options{Object}?) or (dakota{Dakota}, name{String}, schema{Schema}, options{Object}?)
// return: model{Model}
Model.compile = function(dakota, name, schema, validations, options) {
  ...
  if (!nm_.isUndefined(validations)) {
    if (!nm_.isNull(validations) || (validations instanceof lValidations)) {
      ...
      throw new lErrors.Model.InvalidArgument('Argument should be a Validations or null.');
    }
  }
...
function ensureTable(model, callback, options) {
  var name = options && !nm_.isUndefined(options.pluralize) && !options.pluralize ? nm_s.underscored(model._name) : nm_i.pluralize(nm_s.underscored(model._name));
  model._table = new lTable(dakota, name, schema);
