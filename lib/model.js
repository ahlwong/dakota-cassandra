...
Model.prototype.save = function(callback, options) {
  ...
  if (clusteringKey) {
    if (nm_.isString(clusteringKey)) {
      primaryKeys[clusteringKey] = true;
    }
  ...
  var assignments = {};
  nm_.each(this._changes, function(value, column) {
    if (!(!self._exists && self._model._schema._isCounterColumnFamily && primaryKeys[column])) {
  ...
  // execute query
  if (this._exists || this._model._schema._isCounterColumnFamily) {
