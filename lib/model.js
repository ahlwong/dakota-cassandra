...
Model.prototype.validate = function(options) {
  ...
  if (this._update) {
    columns = nm_.reduce(nm_.keys(this._changes), function(memo, column) {
      if (!self._changes[column].op || self._changes[column].op['$set']) {
        memo.push(column);
      }
      return memo;
    }, []);
...
function get(column) {
  ...
  if (this._update && !this._model._schema.isKeyColumn(column)) {
    ...
    else if (this._changes[column].op && !this._changes[column].op['$set']) {
...
function changes(changes, column) {
  ...
  if (column && !nm_.isString(column)) {
    ...
    if (column) {
      ...
      if (!nm_.isUndefined(value)) {
        var newValue = this._get(column);
        return { from: value.prev, to: newValue, op: value.op && !value.op['$set'] ? value.op : { '$set' : newValue } }
      }
      else {
        return value;
      }
      ...
      nm_.each(changes, function(value, column) {
        ...
        c[column] = { from: value.prev, to: newValue, op: value.op && !value.op['$set'] ? value.op : { '$set' : newValue } }
      });
      return c;
    }
  }
