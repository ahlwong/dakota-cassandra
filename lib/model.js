...
Model.prototype.validate = function(options) {
  ...
  var columns = this._update ? nm_.keys(this._changes) : this._model._schema.columns(); // only validate changed columns if update
...
function increment(column, delta) {
  ...
  // if update, only record idempotent operations
  if (this._update) {
    if (this._changes[column] && this._changes[column].op && (this._changes[column].op['$incr'] || this._changes[column].op['$decr'])) {
...
// expects: (where{Object})
// returns: model{Model}
staticMethods.update = function(where) {
