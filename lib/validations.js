...
function Validations(schema, definition, options) {
...
Validations.validate = function(recipe, value, displayName) {
  var messages = null;
  if (!lHelpers.isHash(recipe)) {
  ...
  else if (!nm_.isUndefined(recipe.validator)) {
    
    // normalize
    var validators = recipe.validator;
    if (lHelpers.isHash(validators)) {
...
Validations.sanitize = function(recipe, value) {
  if (!lHelpers.isHash(recipe)) {
    throw new lErrors.Validations.InvalidType('Type should be a {}.');
  }
  else if (!nm_.isUndefined(recipe.sanitizer)) {
    ...
    if (nm_.isArray(sanitizers)) {
      return nm_.reduce(sanitizers, function(value, sanitizer) { return sanitizer(value); }, value);
    }
    else {
      throw new lErrors.Validations.InvalidType('Type should be an array.');
    }
...
Validations.validateSanitized = function(recipe, value, displayName) {
  return Validations.validate(recipe, Validations.sanitize(recipe, value), displayName);
}
...
Validations.prototype.recipe = function(column) {
  if (!nm_.isUndefined(this._definition[column])) {
      ...
      if (key === 'displayName') {
        ...
        if (!nm_.isArray(value)) {
          ...
          nm_.each(value, function(v, index) {
            if (!lHelpers.isHash(v)) {
              throw new lErrors.Validations.InvalidType('Type should be a {}.');
            }
            else if (nm_.isUndefined(v.validator) || !nm_.isFunction(v.validator)) {
              throw new lErrors.Validations.InvalidType('Type should be a function.');
            }
            else if (nm_.isUndefined(v.message) || !nm_.isFunction(v.message)) {
              throw new lErrors.Validations.InvalidType('Type should be a function.');
            }
          });
        }
