...
// lib
var lBucketing = require('./bucketing');
...
function Dakota(options, userDefinedTypes) {
  ...
  init.call(this, userDefinedTypes ? userDefinedTypes : {}, this._options);
}
...
// expects: the same params as Cassandra.Client.stream(); (model{Model}, query{String}, [param, param, ...], options{Object}) or (query{String}, [param, param, ...], options{Object})
// returns: undefined
Dakota.prototype.stream = function(model, query, params, options) {
  ...
  var stream = new lWrappedStream(model);
  if (!this._ready) {
    addToQueryQueue.call(this, 'stream', { stream: stream, args: [query, params, options] });
  }
  else {
    stream._setStream(this._client.stream(query, params, options));
  }
  return stream;
};
      ...
      nm_.each(queue, function(query, index) {
        ...
        if (!systemOnly) {
          if (action === 'execute') {
            ...
            query.stream._setStream(self._client.stream.apply(self, query.args));
          }
        }
      });
    });
