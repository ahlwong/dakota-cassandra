...
// expects: (name{String}, schema{Schema}, validations{Validations}, options{Object})
// return: model{Model}
Dakota.prototype.addModel = function(name, schema, validations, options) {
  if (!nm_.isString(name)) {
    ...
    throw new lErrors.Dakota.InvalidArgument('Argument should be a Validations.');
  }
  else if (!nm_.isUndefined(options) && !lHelpers.isHash(options)) {
    throw new lErrors.Dakota.InvalidArgument('Argument should be a {}.');
  }
  else if (!nm_.isUndefined(this._models[name])) {
