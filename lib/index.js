...
function Dakota(options, userDefinedTypes) {
  ...
  else if (!userDefinedTypes || !lHelpers.isHash(userDefinedTypes)) {
    throw new lErrors.Dakota.InvalidArgument('Argument should be a {}.');
  }
  ...
  init.call(this, userDefinedTypes, options);
}
...
function init(userDefinedTypes, options) {
  ...
  // configure logger
  configureLogger.call(this, options.logging);
  ...
  ensureKeyspace.call(this, function() {
    ...
    processAndEnsureUserDefinedTypes.call(self, userDefinedTypes, function() {
      
      // dakota ready
      self._ready = true;
      processQueryQueue.call(self, false);
      
    }, options.user_defined_types);
  }, options.keyspace);
}
...
//          example options : { replication: replication{Object}, durableWrites: durableWrites{Boolean} }
// returns: undefined
function processAndEnsureUserDefinedTypes(userDefinedTypes, callback, options) {
  var self = this;
  ...
  nm_.each(userDefinedTypes, function(definition, name) {
    if (self._userDefinedTypes[name]) {
      ...
      self._userDefinedTypes[name] = new lUserDefinedType(self, name, definition, {});
    }
  });
...
Dakota.prototype.addModel = function(name, schema, validations, options) {
  ...
  else if (!lHelpers.isHash(schema)) {
    throw new lErrors.Dakota.InvalidArgument('Argument should be a Schema.');
  }
  else if (validations && !lHelpers.isHash(validations)) {
    throw new lErrors.Dakota.InvalidArgument('Argument should be a {}.');
  }
  ...
  if (this._models[name]) {
    ...
    var model = lModel.compile(this, name, new lSchema(schema, options.schema), new lValidations(schema, validations, options.validations), options);
