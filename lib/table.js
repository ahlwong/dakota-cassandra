...
//          recreateColumn{Boolean} : recreate columns where types don't match schema
...
Table.prototype.ensureExists = function(callback, options) {
  ...
  // default options
  options = nm_.extend({ recreate: false, recreateColumn: false, removeExtra: false, addMissing: false }, this._options.ensureExists, options);
  
  // skip running
  if (!nm_.isUndefined(options.run) && !options.run) {
    callback();
    lLogger.debug('Ensure table skipped: ' + this._dakota._keyspace + '.' + this._name + '.');
    return;
  }
    ...
    if (err) {
      ...
      // compare schema to existing table
        ...
        nm_.each(result.rows, function(row, index) {
          ...
          if (columns[column]) {
            var dbValidator = lTypes.dbValidator(self._dakota, self._schema.columnType(column));
        ...
        if ((mismatched.length > 0 || extra.length > 0 || missing.length > 0) && options.recreate) {
          ...
          if (mismatched.length > 0 && options.recreateColumn) {
            promises = promises.concat(fixMismatched.call(self, nm_.map(mismatched, function(mismatch) { return mismatch.column })));
          }
...
function fixMismatched(mismatched) {
  ...
  var promises = [];
  nm_.each(mismatched, function(column, index) {
    promises.push(nmWhen.promise(function(resolve, reject) {
      self.dropColumn(column, function(err, result) {
        if (err) {
          reject(err);
        }
        else {
...
function fixExtra(extra) {
  ...
  var promises = [];
  nm_.each(extra, function(column, index) {
    promises.push(nmWhen.promise(function(resolve, reject) {
      self.dropColumn(column, function(err, result) {
        if (err) {
          reject(err);
