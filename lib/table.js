...
//          fixMismatch[$recreate]{String} : change column types to match schema
  ...
  this.selectSchema(function(err, result) {
    if (err) {
      ...
      if (result.rows.length === 0) {
        console.log('Creating table: ' + self._dakota._keyspace + '.' + self._name + '.');
      ...
      // compare schema to existing table
        ...
        nm_.each(result.rows, function(row, index) {
          ...
          if (columns[name]) {
            var type = self._schema.columnType(name);
            if (self._schema.isColumnCollection(name)) {
              if (lCollections.fullDBValidatorString(type, self._schema.columnCollectionType(name)) != row.validator) {
                mismatched.push(row);
              }
            }
            else if (lTypes.TYPES[type].dbValidator !== row.validator) {
              mismatched.push(row);
            }
            delete columns[row.column_name];
          }
          else {
            extra.push(row);
          }
        ...
        if (mismatched.length > 0) {
          console.log('Found ' + mismatched.length + ' mismatched column types in ' + self._dakota._keyspace + '.' + self._name);
          console.log(mismatched);
        }
        if (extra.length > 0) {
          console.log('Found ' + extra.length + ' extra columns in ' + self._dakota._keyspace + '.' + self._name);
          console.log(extra);
        }
        if (missing.length > 0) {
          console.log('Found ' + missing.length + ' missing columns in ' + self._dakota._keyspace + '.' + self._name);
          console.log(missing);
        }
...
function fixMismatched(mismatched) {
  ...
  console.log('Recreating columns with mismatched types...');
    ...
    promises.push(nmWhen.promise(function(resolve, reject) {
      self.dropColumn(name, function(err, result) {
        if (err) {
          ...
          self.addColumn(name, self._schema.fullColumnTypeString(name), function(err, result) {
            if (err) {
              reject(err);
            }
            else {
...
function fixExtra(extra) {
  ...
  console.log('Removing extra columns...');
...
function fixMissing(missing) {
  ...
  nm_.each(missing, function(column, index) {
    promises.push(nmWhen.promise(function(resolve, reject) {
      self.addColumn(column, self._schema.fullColumnTypeString(column), function(err, result) {
        if (err) {
          reject(err);
        }
        else {
...
// !!! currently not allowed via cassandra-driver
...
function buildColumns() {
  ...
  nm_.each(this._schema.columns(), function(column, index) {
    ...
    clause += '"' + column + '" ' + self._schema.fullColumnTypeString(column);
  });
