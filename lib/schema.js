...
Schema.prototype.isValidValueTypeForColumn = function(column, value) {
  ...
  return lTypes.isValidValueType(this.columnType(column), value);
}
...
function validateAndNormalizeDefinition(definition) {
  if (!lHelpers.isHash(definition)) {
    ...
    nm_.each(definition, function(value, key) {
      if (key !== 'columns' && key !== 'key' && key !== 'with' && key !== 'callbacks' && key !== 'methods' && key !== 'static_methods' && key !== 'options') {
    ...
    if (definition.static_methods) {
      validateAndNormalizeStaticMethods.call(this, definition.static_methods);
    }
  }
}
...
function validateAndNormalizeColumns(columns) {
  if (!lHelpers.isHash(columns)) {
    ...
    nm_.each(columns, function(definition, column) {
      ...
      else if (!definition.type || !nm_.isString(definition.type) || !lTypes.isValidType(definition.type)) {
...
function validateAndNormalizeStaticMethods(static_methods) {
  nm_.each(static_methods, function(static_method, key) {
...
function mixinStaticMethods(model) {
  ...
  if (this._definition.static_methods) {
    nm_.each(this._definition.static_methods, function(static_method, key) {
